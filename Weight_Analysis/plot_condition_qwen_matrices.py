...
VALUE_LABELS = {
    "condition_number": "Condition number relative to base",
    "condition_number_ratio": "cond(checkpoint) / cond(base)",
    "condition_number_relative_change": "(cond(checkpoint) - cond(base)) / cond(base)",
    "condition_number_delta": "cond(checkpoint) - cond(base)",
    "checkpoint_condition_number": "Checkpoint condition number",
    "base_condition_number": "Base condition number",
}
...
def load_condition_csv(csv_path: Path, value_column: str, condition_target: Optional[str]) -> pd.DataFrame:
    if not csv_path.exists():
        raise FileNotFoundError(f"Condition-number CSV does not exist: {csv_path}")
    ...
    if condition_target is not None:
        if "condition_target" not in df.columns:
            raise ValueError("--condition-target was provided, but the CSV has no condition_target column.")
        df = df[df["condition_target"].astype(str) == condition_target].copy()
        if df.empty:
            raise ValueError(f"No rows remain for condition_target={condition_target!r}.")
...
def plot_matrix_group(
    ...
    output_path = output_dir / f"condition_number_{safe_filename(value_column)}_{safe_filename(matrix_name)}.png"
    fig.tight_layout()
...
def plot_matrix_layers(
    ...
    fig.supylabel(label)
    output_path = output_dir / f"condition_number_layers_{safe_filename(value_column)}_{safe_filename(matrix_name)}.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def plot_heatmap(
    ...
    ax.set_title(f"Condition-number heatmap, checkpoint step {step}", fontsize=12)
    ...
    output_path = output_dir / f"condition_number_heatmap_step_{step}_{safe_filename(value_column)}_{safe_filename(mode)}.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Qwen matrix-level condition-number metrics from condition_number_matrices.csv."
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=Path("output/train_ckpt/gp_l_sft-slight-qwen/condition_number_analysis/condition_number_matrices.csv"),
        help="Input CSV generated by compute_condition_qwen_matrices.py.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--value-column",
        choices=CONDITION_COLUMNS,
        default="condition_number",
        help="CSV column to plot. Default: condition_number.",
    )
    parser.add_argument(
        "--condition-target",
        choices=("ratio", "relative-change", "delta", "checkpoint"),
        default=None,
        help="Optional filter for CSVs containing multiple condition_target values.",
    )
    parser.add_argument(
        "--yscale",
        choices=("linear", "log"),
    ...
    parser.add_argument(
        ...
        default="raw",
        help=(
            "Value shown in heatmaps. raw uses the selected condition metric directly; "
...
def main() -> None:
    ...
    if args.condition_target is not None:
        print(f"[INFO] condition target filter: {args.condition_target}")
