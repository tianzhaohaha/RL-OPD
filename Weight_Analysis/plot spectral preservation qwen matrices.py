#!/usr/bin/env python3
"""Plot Qwen matrix-level spectral-preservation metrics.

Reads spectral_preservation_matrices.csv produced by
compute_spectral_preservation_qwen_matrices.py and renders the same plot
family as plot_condition_qwen_matrices.py (per-matrix overview, per-layer
small multiples, per-param curves, and per-step heatmaps).

Selectable value columns (via --value-column):
  principal_angle_mean / principal_angle_max  -> rotation over U and V (deg)
  principal_angle_u_mean / principal_angle_u_max
  principal_angle_v_mean / principal_angle_v_max
  nss                                         -> normalized spectral shift
  metric_value                                -> whatever the compute step pinned
"""
import argparse
...
VALUE_LABELS = {
    "principal_angle_mean": "Principal-angle rotation (deg, mean U & V)",
    "principal_angle_max": "Principal-angle rotation (deg, max U & V)",
    "principal_angle_u_mean": "Left-subspace rotation θ(U) (deg, mean)",
    "principal_angle_u_max": "Left-subspace rotation θ(U) (deg, max)",
    "principal_angle_v_mean": "Right-subspace rotation θ(V) (deg, mean)",
    "principal_angle_v_max": "Right-subspace rotation θ(V) (deg, max)",
    "nss": "Normalized spectral shift (NSS)",
    "metric_value": "Selected spectral-preservation metric",
}
# Short tags used in output filenames so the four plot families stay grouped.
FILE_PREFIX = "spectral"
...
def load_spectral_csv(csv_path: Path, value_column: str, metric_target: Optional[str]) -> pd.DataFrame:
    if not csv_path.exists():
        raise FileNotFoundError(f"Spectral-preservation CSV does not exist: {csv_path}")

    df = pd.read_csv(csv_path)
    required = {"step", "layer", "block", "matrix", "param_name", value_column}
    ...
    if metric_target is not None:
        if "metric_target" not in df.columns:
            raise ValueError("--metric-target was provided, but the CSV has no metric_target column.")
        df = df[df["metric_target"].astype(str) == metric_target].copy()
        if df.empty:
            raise ValueError(f"No rows remain for metric_target={metric_target!r}.")
    ...
    if df.empty:
        raise ValueError(f"No finite {value_column} values remain after loading {csv_path}.")
    return df
...
def value_label(value_column: str) -> str:
    return VALUE_LABELS.get(value_column, value_column)
...
def plot_matrix_group(
    ...
    mean_df = matrix_df.groupby("step", as_index=False)[value_column].mean().sort_values("step")
    ax.plot(
        mean_df["step"],
        mean_df[value_column],
        color="black",
        marker="o",
        linewidth=2.4,
        markersize=4.5,
    ...
    label = value_label(value_column)
    ax.set_title(f"{label} vs checkpoint step: {matrix_name}")
    ...
    output_path = output_dir / f"{FILE_PREFIX}_{safe_filename(value_column)}_{safe_filename(matrix_name)}.png"
    fig.tight_layout()
...
def plot_param(
    ...
    ax.set_title(f"{label}: layer {layer} {matrix_name}")
    ax.set_xlabel("Checkpoint step")
    ax.set_ylabel(label)
    ax.set_yscale(yscale)
    ax.grid(True, linestyle="--", linewidth=0.6, alpha=0.35)
...
def plot_matrix_layers(
    ...
    label = value_label(value_column)
    fig.suptitle(f"{label} by layer: {matrix_name}", fontsize=14)
    fig.supxlabel("Checkpoint step")
    fig.supylabel(label)
    output_path = output_dir / f"{FILE_PREFIX}_layers_{safe_filename(value_column)}_{safe_filename(matrix_name)}.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def heatmap_values(
    ...
    values = step_df.pivot_table(index="matrix", columns="layer", values=value_column, aggfunc="mean")
    ...
    if mode == "relative-baseline":
        ...
        baseline = base_df.pivot_table(index="matrix", columns="layer", values=value_column, aggfunc="mean")
...
def heatmap_colorbar_label(value_column: str, mode: str, baseline_step: int) -> str:
    if mode == "raw":
        return value_label(value_column)
    if mode == "relative-layer-mean":
        return f"({value_column} - row mean) / row mean"
    if mode == "relative-baseline":
        return f"({value_column} - step {baseline_step}) / step {baseline_step}"
    if mode == "zscore-layer":
...
def plot_heatmap(
    ...
    ax.set_title(f"{value_label(value_column)} heatmap, checkpoint step {step}", fontsize=12)
    ...
    cbar.set_label(heatmap_colorbar_label(value_column, mode, baseline_step), labelpad=8)

    output_path = output_dir / f"{FILE_PREFIX}_heatmap_step_{step}_{safe_filename(value_column)}_{safe_filename(mode)}.png"
    fig.tight_layout()
    fig.savefig(output_path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return output_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot Qwen matrix-level spectral-preservation metrics from spectral_preservation_matrices.csv."
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=Path(
            "output/train_ckpt/gp_l_sft-slight-qwen/spectral_preservation_analysis/spectral_preservation_matrices.csv"
        ),
        help="Input CSV generated by compute_spectral_preservation_qwen_matrices.py.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        default="principal_angle_mean",
        help="CSV column to plot. Default: principal_angle_mean.",
    )
    parser.add_argument(
        "--metric-target",
        choices=("principal-angle-mean", "principal-angle-max", "principal-angle-u-mean", "principal-angle-v-mean", "nss"),
        default=None,
        help="Optional filter for CSVs containing multiple metric_target values.",
    )
    parser.add_argument(
        "--yscale",
        choices=("linear", "log"),
    ...
    parser.add_argument(
        ...
        default="raw",
        help=(
            "Value shown in heatmaps. raw uses the selected metric directly; "
...
def main() -> None:
    ...
    print(f"[INFO] value column: {args.value_column}")
    if args.metric_target is not None:
        print(f"[INFO] metric target filter: {args.metric_target}")
